...
describe('A Star', () => {
  ...
  it('finds a path between two cells', () => {
    ...
    const actualPath = testAStar.aStar(testGrid.startingPoint, testGrid.finishingPoint, gridRows, gridCols);
  ...
  it('finds a path between two other cells', () => {
    ...
    const actualPath = testAStar.aStar(testGrid.startingPoint, testGrid.grid[21], gridRows, gridCols);
    expect(actualPath).toEqual(expectedPath);
  });
});
